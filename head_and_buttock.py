...
import time
import numpy as np
from Tools import inference2, initialize_detector, initialize_det_segmentation, video_realease, \
    video_prepare, draw_x_rectangle, draw_y_rectangle, get_headtop, get_buttocks_densepose, get_head_densepose, \
    get_densepose_info, filter_and_select_closest_instance


def main(input_video_path, output_video_path, f_idx, interval, ball_position):
    ...
    while True:
        ...
        buttocks_coords = get_buttocks_densepose(iuv_array, box)
        ...
        if abs(x1 - prev_x1) > 10 or abs(y1 - prev_y1) > 10:
            x1 = prev_x1
            y1 = prev_y1
        if abs(x2 - prev_x2) > 10 or abs(y2 - prev_y2) > 10:
            x2 = prev_x2
            y2 = prev_y2
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--input_video_path", type=str, default="./data/side548.mp4"
    )
    parser.add_argument(
        "-o", "--output_video_path", type=str, default="./output/output_548.mp4"
    )
    parser.add_argument("-f", "--f_idx", type=int, default=548)
    ...
    # 主函数
    main(args.input_video_path, args.output_video_path, args.f_idx, args.interval, None)
